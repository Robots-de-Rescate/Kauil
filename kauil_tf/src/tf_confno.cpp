    ...
    odom_broadcaster.sendTransform(  //mandamos la transformada de odom a base_link
      tf::StampedTransform(
        ...
        tf::Vector3(odomsg->pose.pose.position.x, 
				...
				odomsg->header.stamp, "/odom", "/base_kauil")); 
...
int main(int argc, char** argv){
  ...
  //subscribe to pose info
	ros::Subscriber pose_sub = n.subscribe<nav_msgs::Odometry>("MD03odom", 1, poseCallback); //subscribes to MD03 odometry
    ...
    //base_link => base_scan
		broadcaster.sendTransform(
			tf::StampedTransform(
				...
				ros::Time::now(), "/base_kauil", "/base_laser"));
