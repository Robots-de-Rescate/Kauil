...
void laserCallback(const boost::shared_ptr<const LaserScan>& scan) {
    ...
    ROS_INFO("Value of angle_max: %f \n", MAX_ANGLE);
    double MIN_ANGLE = scan->angle_min; //start angle of the scan
    ROS_INFO("Value of angle_min: %f \n", MIN_ANGLE);
    ...
    for (int bin = 0; bin < bins; bin++) {
        ...
        ROS_INFO("The angle in this bin %i is: %f \n", bin, thisangle);
        double range = scan->ranges[bin];//range data
        ROS_INFO("The range in this bin %i is: %f \n", bin, range);
...
int main(int argc, char** argv){
    ...
    ros::Publisher cmdVelpublisher = n.advertise<geometry_msgs::Twist>("/RosAria/cmd_vel", 50);
